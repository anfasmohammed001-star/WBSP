        ...
        <div className="bg-background min-h-screen pb-24 transition-colors duration-300 text-foreground max-w-md mx-auto shadow-2xl overflow-hidden border-x border-border">
            ...
            <div className="p-6 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4">
                    {[
                        { label: 'Community', icon: User, color: 'text-purple-500', bg: 'bg-purple-500/10', path: '/community' },
                        { label: 'Guides', icon: HelpCircle, color: 'text-orange-500', bg: 'bg-orange-500/10', path: '/guides' },
                        { label: 'AR Help', icon: WalletIcon, color: 'text-blue-500', bg: 'bg-blue-500/10', path: '/ar-visualizer' },
                        { label: 'Bundles', icon: ShieldCheck, color: 'text-green-500', bg: 'bg-green-500/10', path: '/bundles' },
                        { label: 'Insights', icon: TrendingUp, color: 'text-pink-500', bg: 'bg-pink-500/10', path: '/insights' },
                        { label: 'Expert Call', icon: Video, color: 'text-indigo-500', bg: 'bg-indigo-500/10', path: '/consultation' },
                        { label: 'Compare', icon: User, color: 'text-cyan-500', bg: 'bg-cyan-500/10', path: '/compare' },
                        { label: 'Gallery', icon: ShieldCheck, color: 'text-rose-500', bg: 'bg-rose-500/10', path: '/gallery' },
                        { label: 'Live Slots', icon: Clock, color: 'text-teal-500', bg: 'bg-teal-500/10', path: '/availability' },
                    ].map((cat) => (
                        <motion.button
                            key={cat.label}
                            whileHover={{ y: -4 }}
